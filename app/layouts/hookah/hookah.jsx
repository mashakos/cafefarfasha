...
import styles from './hookah.module.css';
...
export const Hookah = ({ children, timecode }) => {
  ...
  // const placeholder = data.hookah.banner_placeholder;
  /*
  ...
  */
  const placeholder = `/cdn-cgi/image/width=25,quality=75/${data.hookah.banner}`;
    ...
    <article className={styles.post}>
        ...
        {data.hookah.banner && (
          <div className={styles.banner} ref={imageRef}>
            <div className={styles.bannerImage}>
              <Image role="presentation" src={data.hookah.banner} placeholder={placeholder} alt="" />
            </div>
        ...
        <header className={styles.header}>
          <div className={styles.headerText}>
            <Heading level={2} as="h1"
                     ...
                     aria-label={data.hookah.title}
                     data-tina-field={tinaField(data.hookah, "title")}>
              {data.hookah.title.split(' ').map((word, index) => (
                <span className={styles.titleWordWrapper} key={`${word}-${index}`}>
                  ...
                  >
                    {word}
                    {index !== data.hookah.title.split(' ').length - 1 ? ' ' : ''}
                  </span>
                </span>
              ))}
            </Heading>
      ...
      <Section className={styles.wrapper} id="postContent" tabIndex={-1}>
        <Text as="div"
              ...
              data-tina-field={tinaField(data.hookah, "abstract")}
        >
          {/*json post object for testing*/}
          {/*{JSON.stringify(data.hookah, null, 2)}*/}
          {/*{console.log(JSON.stringify(data.hookah, null, 2))}*/}
          {data.hookah.abstract}
          <br/>
          {data.hookah.price}
        </Text>
      </Section>
