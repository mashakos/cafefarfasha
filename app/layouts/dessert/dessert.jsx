...
export const Dessert = ({ children, timecode }) => {
  ...
  // const placeholder = data.drink.banner_placeholder;
  /*
  ...
  */
  const placeholder = `/cdn-cgi/image/width=25,quality=75/${data.drink.banner}`;
    ...
    <article className={styles.post}>
        ...
        {data.drink.banner && (
          <div className={styles.banner} ref={imageRef}>
            <div className={styles.bannerImage}>
              <Image role="presentation" src={data.drink.banner} placeholder={placeholder} alt="" />
            </div>
        ...
        <header className={styles.header}>
          <div className={styles.headerText}>
            <Heading level={2} as="h1"
                     ...
                     aria-label={data.drink.title}
                     data-tina-field={tinaField(data.drink, "title")}>
              {data.drink.title.split(' ').map((word, index) => (
                <span className={styles.titleWordWrapper} key={`${word}-${index}`}>
                  ...
                  >
                    {word}
                    {index !== data.drink.title.split(' ').length - 1 ? ' ' : ''}
                  </span>
                </span>
              ))}
            </Heading>
      ...
      <Section className={styles.wrapper} id="postContent" tabIndex={-1}>
        <Text as="div"
              ...
              data-tina-field={tinaField(data.drink, "abstract")}
        >
          {/*json post object for testing*/}
          {/*{JSON.stringify(data.drink, null, 2)}*/}
          {/*{console.log(JSON.stringify(data.drink, null, 2))}*/}
          {data.drink.abstract}
          <br/>
          {data.drink.price}
        </Text>
